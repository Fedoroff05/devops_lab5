...
def test_create_user_with_invalid_email():
    response = client.post("/api/v1/user", json={"name": "Existing User", "email": "i.i.ivanov@mail.com"})
    assert response.status_code == 409
    assert response.json() == {"detail": "User with this email already exists"}


def test_delete_user():
    create_response = client.post("/api/v1/user", json={"name": "To Delete", "email": "todelete@mail.com"})
